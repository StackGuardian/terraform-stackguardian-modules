...
- `network.create_network_infrastructure` - Create additional network resources (default: false)
...
- `firewall.ssh_access_rules` - IP ranges allowed SSH access (optional, object with CIDR blocks)
- `firewall.additional_ingress_rules` - Custom firewall rules for additional ports (optional, object)
...
- `scaling.scale_in_threshold` - Scale in when queue drops below this many jobs (default: 1, minimum: 1)
