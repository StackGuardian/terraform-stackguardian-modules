...
resource "aws_iam_role" "runner" {
  name = "${local.effective_prefix}-ec2-private-runner-role"
...
resource "aws_iam_policy" "ec2_runner_assume_s3_role" {
  name        = "${local.effective_prefix}-ec2-runner-assume-s3-role-policy"
...
resource "aws_iam_instance_profile" "this" {
  name = "${local.effective_prefix}-runner-instance-profile"
  role = aws_iam_role.runner.name
}
